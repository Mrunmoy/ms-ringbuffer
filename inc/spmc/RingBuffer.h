...
//
// Structural mirror of MPSC: single producer writes without CAS,
// multiple consumers CAS on tail to claim slots. Per-slot sequence
// counters for safe coordination.
...
namespace ouroboros::spmc
{
    ...
    public:
        ...
        [[nodiscard]] bool push(const T &item)
        {
            ...
            m_ctrl.head.store(pos + 1, std::memory_order_release);
            return true;
        }
        ...
        [[nodiscard]] bool pop(T &item)
        {
            ...
            for (;;)
            {
                Slot<T> &slot = m_slots[pos & Mask];
                uint32_t seq = slot.sequence.load(std::memory_order_acquire);
                auto diff = static_cast<int32_t>(seq - (pos + 1));

                if (diff == 0)
                {
                    if (m_ctrl.tail.compare_exchange_weak(
                            pos, pos + 1,
                ...
                else
                {
                    ...
                    pos = m_ctrl.tail.load(std::memory_order_relaxed);
                }
            }
        }
